...
This script provides a command-line interface for the Cylex scraper.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/cylex_cli.log'),
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Cylex Scraper CLI')
    ...
    parser.add_argument('--output', '-o', type=str, help='Output filename (default: cylex_[country]_[query].json)')
    ...
    if args.output:
        ...
        output_filename = f"cylex_{args.country}_{args.query.replace(' ', '_')}.json"
    
    logger.info(f"Starting Cylex scraper with query: {args.query}, location: {args.location}")
    
    try:
        # Create and run the scraper
        scraper = CylexScraper(
        ...
        logger.info(f"Found {len(results)} results from Cylex")
        ...
        logger.error(f"Error running Cylex scraper: {e}", exc_info=True)
        return 1
